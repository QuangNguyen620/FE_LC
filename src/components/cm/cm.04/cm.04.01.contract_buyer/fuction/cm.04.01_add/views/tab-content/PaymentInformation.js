...
const ConstractInfomation = ({ lang = 'vi' }) => {
            ...
            >
              ...
              >
                {context?.licenseOptions.map((license, i) => (
                  <Option key={license.key} value={i}>
                    {license.label}
                  </Option>
                ))}
              </Select>
  ...
  const changeLicenseId = (e, key) => {
    ...
    dataSource[index].licenseId = context?.licenseOptions[e].value;
    dataSource[index].licenseName = context?.licenseOptions[e].label;
            ...
            <Row>
              <Col span={23}>
                ...
                {/* <Row>
                  ...
                  <Col span={16}>
                    {licenseList?.map((addendum, index) => (
                      <div>
                        <Row
                          key={addendum?.key}
                          style={{ display: 'flex' }}
                        ...
                        >
                          <Col span={22}>
                            <Form.Item
                              name={'licenseId' + addendum?.key}
                              rules={[
                                {
                                  required: true,
                                  message: 'Bộ chứng từ không được bỏ trống',
                                },
                              ]}
                            >
                              <Select
                                name={'licenseId' + addendum?.key}
                                options={context?.licenseOptions}
                          ...
                          <Col span={1}>
                            <MinusCircleOutlined
                              style={{
                                display:
                                  licenseList.length == addendum?.key
                                    ? 'flex'
                                    : 'none',
                              }}
                              onClick={() => remove(addendum?.key)}
                            />
                          </Col>
                        </Row>
                      </div>
                    ...
                    <Col span={24}>
                      <Space className="button-add-license">
                        <Button
                          onClick={() => add()}
