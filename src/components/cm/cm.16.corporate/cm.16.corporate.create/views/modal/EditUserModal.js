import React, { useState, useEffect } from 'react';
import {
  Col,
  Row,
  Input,
  Button,
  Form,
  Select,
  Modal,
  Radio,
  Space,
  DatePicker,
} from 'antd';
import moment from 'moment';
import { KTTitle } from 'core/ui';
import { useTranslation } from 'react-i18next';
import {} from '@ant-design/icons';
import '../CreateCorporate.less';
import '../../../../../../assets/less/LC-common.less';
import { CreateCorporateDomain } from '../../domains/CreateCorporateDomain';

var axios = require('axios');
const listRoles = [
  { label: 'Maker', value: 'Maker' },
  { label: 'Approver', value: 'Approver' },
  { label: 'Maker+Approver', value: 'Maker+Approver' },
];

const listPositions = [
  { label: 'Người đại diện pháp luật', value: 'legal_representative' },
  { label: 'Kế toán trưởng', value: 'accountant' },
  { label: 'Nhân viên', value: 'employee' },
  { label: 'Trưởng phòng', value: 'head_of_department' },
];

const EditUserModal = ({ lang = 'vi', ...props }) => {
  const [userInfo, setUserInfo] = useState(null);

  const { t } = useTranslation();
  const [context, domain] = CreateCorporateDomain();
  const dateFormatList = 'DD/MM/YYYY';
  const exceptThisSymbols = ['e', 'E', '+', '-', '.'];

  const [listGroupCode, setListGroupCode] = useState([]);

  const [form] = Form.useForm();

  function onModalClose() {
    let data = { name: 'example', type: 'closed from child' };
    form.resetFields();
    props.onCloseModal(data);
  }

  function onChangeUserAccount(e) {
    props.onChangeUserAccount(e);
  }
  function onChangeUserRole(e) {
    setUserInfo({ ...userInfo, role: e });
    getUserGroupCode(e);
  }

  function onChangeUserGroup(e) {
    setUserInfo({ ...userInfo, userGroupCode: e });
  }

  function onChangeIdentityType(e) {
    props.onChangeIdentityType(e);
  }

  const onDateOfIdentityChange = (e) => {
    //props.onChangeDateOfIdentity(e);
  };

  function onChangeUserPosition(e) {
    props.onChangeUserPosition(e);
  }

  const onFinishFailed = (errorInfo) => {
    console.log('Failed:', errorInfo);
  };

  const submitHandler = (e) => {
    // var data = props.bankAccount;
    let data = { name: 'example', type: 'submit from child' };
    domain.editUserInformation(e);
    props.onCloseModal(data);
  };

  const getUserGroupCode = (role) => {
    var config = domain.getAllUserGroup();

    config
      .then((result) => {
        axios(result)
          .then(function (response) {
            console.log('Tham số đầu vào là vai trò của người dùng::::');
            console.log(role);
            console.log(response.data.data);
            var tempArr = [];

            response.data.data.forEach((element) => {
              if (element.userType === 'CORPORATE') {
                if (role != 'Maker+Approver') {
                  if (element.role == role) {
                    tempArr.push(element);
                  }
                } else {
                  tempArr.push(element);
                }
              }
            });
            console.log('Nhóm người dùng tương ứng::::');
            console.log(tempArr);
            setListGroupCode(tempArr);
          })
          .catch(function (error) {
            console.log(error.data);
          });
      })
      .catch((err) => console.log(err));
  };

  useEffect(() => {
    setUserInfo(props.userInfo);
    form.setFieldsValue({
      userName: props.userInfo?.userName || '',
      position: props.userInfo?.position || '',
      identityType: props.userInfo?.identityType || '',
      identityNumber: props.userInfo?.identityNumber || '',
      dateOfIdentity: moment(
        props.userInfo?.dateOfIdentity || '01/01/2015',
        dateFormatList,
      ),
      issuedByIdentity: props.userInfo?.issuedByIdentity || '',
      phoneNumber: props.userInfo?.phoneNumber || '',
      email: props.userInfo?.email || '',
      role: props.userInfo?.role || '',
      userGroupCode: props.userInfo?.userGroupCode || '',
    });
  }, [props.userInfo]);

  useEffect(() => {
    const getUserGroupCode = (role) => {
      var config = domain.getAllUserGroup();

      config
        .then((result) => {
          axios(result)
            .then(function (response) {
              console.log('Tham số đầu vào là vai trò của người dùng::::');
              console.log(role);
              console.log(response.data.data);
              var tempArr = [];

              response.data.data.forEach((element) => {
                if (element.userType === 'CORPORATE') {
                  if (role != 'Maker+Approver') {
                    if (element.role == role) {
                      tempArr.push(element);
                    }
                  } else {
                    tempArr.push(element);
                  }
                }
              });
              console.log('Nhóm người dùng tương ứng::::');
              console.log(tempArr);
              setListGroupCode(tempArr);
            })
            .catch(function (error) {
              console.log(error.data);
            });
        })
        .catch((err) => console.log(err));
    };

    getUserGroupCode(props.userInfo?.role);
  }, [props.userInfo?.role]);

  return (
    <>
      <Modal
        centered
        visible={props.isVisbled}
        onCancel={onModalClose}
        footer={[
          <Button
            className="common-btn"
            htmlType="submit"
            form="myForm"
            key="submit"
          >
            Xác nhận
          </Button>,
          <Button onClick={onModalClose} className="secondary-btn">
            Đóng
          </Button>,
        ]}
        size={3}
      >
        <KTTitle size={3}>
          <b>Sửa thông tin người dùng</b>
        </KTTitle>
        <Form onFinish={submitHandler} id="myForm" form={form}>
          <Row style={{ paddingTop: 13 }}>
            <Col span={24}>
              <Row>
                <Col span={8}>
                  Tên hiển thị <span style={{ color: '#F5222D' }}>*</span>
                </Col>
                <Col span={16}>
                  <Form.Item
                    name={'userName'}
                    rules={[
                      {
                        required: true,
                        message: 'Trường tên hiển thị không được để trống',
                      },
                      {
                        whitespace: true,
                        message: 'Trường tên hiển thị không được để trống',
                      },
                      {
                        max: 50,
                        message:
                          'Trường tên hiển thị không được phép vượt quá 50 ký tự!',
                      },
                    ]}
                  >
                    <Input
                      name="userName"
                      onChange={onChangeUserAccount}
                      allowClear
                      value={userInfo?.userName}
                    />
                  </Form.Item>
                </Col>
              </Row>
              <Row>
                <Col span={8}>
                  Chức vụ <span style={{ color: '#F5222D' }}>*</span>
                </Col>
                <Col span={16}>
                  <Form.Item
                    name={'position'}
                    rules={[
                      {
                        whitespace: true,
                        required: true,
                        message: 'Trường chức vụ không được để trống',
                      },
                    ]}
                  >
                    <Select name={'position'} onChange={onChangeUserPosition}>
                      {listPositions.map((position) => (
                        <Select.Option value={position.value}>
                          {position.label}
                        </Select.Option>
                      ))}
                    </Select>
                    {/* <Input name="position" onChange={onChangeUserAccount} /> */}
                  </Form.Item>
                </Col>
              </Row>
              <Row>
                <Col span={8}>Giấy tờ định danh</Col>
                <Col span={16}>
                  <Form.Item name={'identityType'}>
                    <Radio.Group
                      defaultValue={'CCCD/CMND'}
                      onChange={onChangeIdentityType}
                    >
                      <Space direction="vertical">
                        <Radio value={'CCCD/CMND'}>CMND/CCCD</Radio>
                        <Radio value={'Hộ chiếu'}>Hộ chiếu</Radio>
                      </Space>
                    </Radio.Group>
                  </Form.Item>
                </Col>
              </Row>
              <Row>
                <Col span={8}>Số giấy tờ định danh </Col>
                <Col span={16}>
                  <Form.Item name={'identityNumber'}>
                    <Input
                      type={'number'}
                      onKeyDown={(e) =>
                        exceptThisSymbols.includes(e.key) && e.preventDefault()
                      }
                      name="identityNumber"
                      onChange={onChangeUserAccount}
                      value={userInfo?.identityNumber}
                    />
                  </Form.Item>
                </Col>
              </Row>

              <Row>
                <Col span={8}>Ngày cấp</Col>
                <Col span={16}>
                  <Form.Item name={'dateOfIdentity'}>
                    <DatePicker
                      style={{ width: '100%' }}
                      defaultValue={userInfo?.dateOfIdentity}
                      format={dateFormatList}
                      onChange={onDateOfIdentityChange}
                      allowClear
                      value={userInfo?.dateOfIdentity}
                    />
                  </Form.Item>
                </Col>
              </Row>
              <Row>
                <Col span={8}>
                  Nơi cấp <span style={{ color: '#F5222D' }}></span>
                </Col>
                <Col span={16}>
                  <Form.Item name={'issuedByIdentity'}>
                    <Input
                      name="issuedByIdentity"
                      onChange={onChangeUserAccount}
                      allowClear
                      value={userInfo?.issuedByIdentity}
                    />
                  </Form.Item>
                </Col>
              </Row>
              <Row>
                <Col span={8}>
                  Số điện thoại <span style={{ color: '#F5222D' }}>*</span>
                </Col>
                <Col span={16}>
                  <Form.Item
                    name={'phoneNumber'}
                    rules={[
                      {
                        required: true,
                        message: 'Trường Số điện thoại không được để trống',
                      },
                      {
                        whitespace: true,
                        message: 'Trường Số điện thoại không được để trống',
                      },
                      {
                        max: 10,
                        message:
                          'Trường Số điện thoại không được phép vượt quá 10 ký tự!',
                      },
                    ]}
                  >
                    <Input
                      name="phoneNumber"
                      onChange={onChangeUserAccount}
                      allowClear
                      value={userInfo?.phoneNumber}
                    />
                  </Form.Item>
                </Col>
              </Row>
              <Row>
                <Col span={8}>
                  Hòm thư điện tử <span style={{ color: '#F5222D' }}>*</span>
                </Col>
                <Col span={16}>
                  <Form.Item
                    name={'email'}
                    rules={[
                      {
                        required: true,
                        message: 'Trường hòm thư điện tử không được để trống',
                      },
                      {
                        whitespace: true,
                        message: 'Trường hòm thư điện tử không được để trống',
                      },
                      {
                        max: 50,
                        message:
                          'Trường hòm thư điện tử không được phép vượt quá 50 ký tự!',
                      },
                    ]}
                  >
                    <Input
                      name="email"
                      onChange={onChangeUserAccount}
                      type="email"
                      allowClear
                      value={userInfo?.email}
                    />
                  </Form.Item>
                </Col>
              </Row>
              <Row>
                <Col span={8}>
                  Vai trò <span style={{ color: '#F5222D' }}>*</span>
                </Col>
                <Col span={16}>
                  <Form.Item
                    name={'role'}
                    rules={[
                      {
                        whitespace: true,
                        required: true,
                        message: 'Trường vai trò không được để trống',
                      },
                    ]}
                  >
                    <Select name={'role'} onChange={onChangeUserRole}>
                      {listRoles.map((role) => (
                        <Select.Option value={role.value}>
                          {role.label}
                        </Select.Option>
                      ))}
                    </Select>
                  </Form.Item>
                </Col>
              </Row>
              <Row>
                <Col span={8}>
                  Tên nhóm người dùng{' '}
                  <span style={{ color: '#F5222D' }}>*</span>
                </Col>
                <Col span={16}>
                  <Form.Item
                    name={'userGroupCode'}
                    rules={[
                      {
                        whitespace: true,
                        required: true,
                        message: 'Trường nhóm người dùng không được để trống',
                      },
                    ]}
                  >
                    <Select name={'userGroupCode'} onChange={onChangeUserGroup}>
                      {listGroupCode.map((userGroup) => (
                        <Select.Option value={userGroup.userGroupCode}>
                          {userGroup.userGroupName}
                        </Select.Option>
                      ))}
                    </Select>
                  </Form.Item>
                </Col>
              </Row>
            </Col>
          </Row>
        </Form>
      </Modal>
    </>
  );
};

export default EditUserModal;
