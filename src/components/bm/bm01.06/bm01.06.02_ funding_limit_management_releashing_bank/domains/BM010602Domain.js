...
import BM010601ContextService from '../services/BM010602ContextService';
...
export function BM010601Domain() {
  ...
  const initDomain = async () => {
    var contextData = {
      ...
      filterData: {
        ...
        statusForSearchRealeaseBank: 'Hiệu lực',
      },
    };
  ...
  const getSearchResult = async () => {
    ...
    var statusForSearchRealeaseBank =
      contextRef?.current?.filterData?.statusForSearchRealeaseBank;

    getFundingLimitReleaseBankList(
      bankId,
      typeLimit,
      moneyType,
      financingLimitCode,
      ...
      dateRangeFrom,
      expirationDateTo,
      statusForSearchRealeaseBank,
    );
  };

  const getFundingLimitReleaseBankList = async (
    bankId,
    typeLimit,
    moneyType,
    financingLimitCode,
    ...
    var data = {
      ...
      statusForSearchRealeaseBank: statusForSearchRealeaseBank,
    };
    var config = {
      method: 'post',
      url:
    ...
    if (response.data.data == null || response.data.data == undefined) {
      ...
      if (response.data.data.length > 0) {
        response.data.data.forEach((element) => {
          if (element.status == 2) {
            var statusString = '';
            var expirationDate = new Date(element.expirationDate);
            var currentDate = new Date();

            if (expirationDate >= currentDate) {
              statusString = (
                <Tag className="custom-tag custom-tag-green">Hiệu lực</Tag>
              );
            } else if (expirationDate < currentDate) {
              statusString = (
                <Tag className="custom-tag custom-tag-red">Hết hiệu lực</Tag>
              );
            }
            ...
            var fundingLimit = {
              ...
              sponsorBank: element.sponsorBank.bankName,
              ...
              availabilityLimit: formatCash(
                element.availabilityLimit.toString(),
              ),
  ...
  const toViewPage = async (id) => {
    history.push(`/bm-home/funding-limit-management-release-bank/view/${id}`);
  };
